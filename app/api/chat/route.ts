...
import { appendAssistantCompletion, persistPromptSnapshot } from "@/lib/conversation-store"
...
const DEFAULT_MODEL = process.env.DEDALUS_MODEL?.trim() || "openai/gpt-4o-mini"
...
function toDedalusMessages(messages: UIMessage[], systemPrompt: string): DedalusChatMessage[] {
  const dedalusMessages: DedalusChatMessage[] = [{ role: "system", content: systemPrompt }]
...
async function streamDedalusCompletion(
  ...
  const response = await fetch(`${DAEDALUS_API_BASE_URL}/chat/completions`, {
    ...
    body: JSON.stringify({
      model,
      messages: toDedalusMessages(messages, DAEDALUS_SYSTEM_PROMPT),
      stream: true,
    }),
    signal: abortSignal,
  })
